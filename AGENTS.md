...
- `betterlectio bypass engaged` — user pressed sidebar "vis original Lectio" escape hatch. Strong signal a redesign is broken. Rich props: identity, page context, settings, display, any visible Lectio error popup. Paired `captureException` with `source: 'bypass_button'`. Flushed synchronously before reload (1500ms cap).
