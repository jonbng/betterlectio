...
- `components/MobileAppDrawer.tsx` - Bottom-right floating drawer that pitches the iOS BetterLectio app to eligible students. Visible only when `students.app_eligible=true && has_app=false && has_android=false`. Collapsed = small `Smartphone` button; expanded = horizontal panel with QR code (via `qrcode` lib) pointing to the App Store, plus a single "Jeg er på Android" CTA that flips `has_android=true` so the prompt is permanently suppressed. `has_app` is set automatically when a student logs into the mobile app — no client-side CTA for that side. Mounted in `entrypoints/content.tsx`'s `DashboardLayout` sibling to `<Toaster />`. Shares `APP_STORE_URL` + `renderAppStoreQrSvg()` with the invite popup via `lib/mobile-app.ts`.
- `components/MobileAppInvitePopup.tsx` - Centered "early access" invite dialog for the same eligible students as `MobileAppDrawer` (identical eligibility fields). Shown once on page load, then re-prompted **7 days later** if the student still hasn't acted on it. Suppressed during quiet hours (02:00–09:00 local) and while the student is currently in a class (`getCountdownState` from `ScheduleCountdown` against `getCachedSchedule`/`getTodaySchedule`). Snooze timestamp is stored in localStorage at `bl-mobile-app-invite-last-shown:{studentId}` (per-device, no migration). Closing via the primary CTA / X / outside-click / Esc soft-snoozes; "Jeg er på Android" writes `marked_android_at` on the student row (same field the drawer uses) so both surfaces hide permanently. Mounted as a sibling to `<MobileAppDrawer />` in `DashboardLayout`. Helpers live in `lib/mobile-app.ts` (`isInviteSnoozed`, `stampInviteShown`, `getInviteSnoozeAt`).
...
- `lib/mobile-app.ts` - Shared App Store URL + `renderAppStoreQrSvg()` QR helper used by both `MobileAppDrawer` and `MobileAppInvitePopup`. Also owns the per-student invite snooze in localStorage (`isInviteSnoozed`, `stampInviteShown`, `getInviteSnoozeAt`) — 7-day cooldown for the popup.
...
- `mobile_app_invite_dismissed` — soft snooze (close / outside-click / Esc / primary CTA on the invite popup). Re-prompts after 7 days.
