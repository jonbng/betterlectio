...
- `entrypoints/elevfeedback-frame.content.ts` - `document_start` + `all_frames` chrome strip for the Elevfeedback editor iframe (`window.name === bl-elevfeedback-editor`). Its MutationObserver is bootstrap-only: it stops at load (10s safety cap), because observing CKEditor's live subtree continuously causes mutation/rAF loops. Do not use srcdoc — CKEditor and ASP.NET save need the live Lectio page.
- `entrypoints/session-block.content.ts` - Blocks session timeout popup

### Components

- `components/FeedbackWidget.tsx` + `components/FeedbackInbox.tsx` - Tall, conversation-first feedback surface. It preserves screenshot-assisted submission and exposes the authenticated user's cross-platform request inbox with previews, unread replies, status context, admin/user chat bubbles, optimistic replies, and an auto-growing keyboard-friendly composer. The floating trigger carries a live numbered unread badge, refreshed on mount, foreground return, and a visibility-aware 60-second interval. Reads and writes use the ownership-checking feedback RPCs.
- `components/AppSidebar.tsx` - Default sidebar navigation with collapsible sections; student names prefer Supabase `name`, while avatars prefer approved `custom_pfp_url` then live Lectio DOM data
...
- `components/LektierPage.tsx` - Day-grouped homework cards with Supabase-backed done-state sync (optimistic local toggle, cross-device persistence). Completion state fresh-loads on mount/foreground return and then polls every 60 seconds only while the page is visible; it does not use Realtime.
- `components/OpgaverPage.tsx` - Single chronological assignment timeline grouped by week, defaulting to the current school year. Its subject pills are scoped to the chosen year, collapse after two measured rows behind a "Vis alle fag" control, and group yearly hold variants through `getSubjectIdentityKey` (for example, `1x FY` and `2x fy` are one Fysik filter) while rows retain their exact Lectio hold. Includes auto-scroll to current week, status indicators, fravær badges, hold pills, real grade badges (Lectio's `--`/dash placeholders are excluded), ignore-missing toggle, and combined elevtimer per week.
- `components/OpgaveDetailSheet.tsx` - Assignment detail side sheet/modal with shared presentation for teacher notes, files, feedback, history, and group submissions. `Opgavenote` is assignment content and must never be used as teacher-feedback fallback; file-only teacher returns are labeled **Rettet version**, while actual `Karakternote`/`Elevnote`/teacher comments use **Lærerens kommentar**. Group projects select status/feedback from the requested `elevid`, mark the current student, keep empty/exhausted group editors visible, and share the same unclipped searchable member picker in both layouts. Comment/file upload uses valid keyboard-accessible controls. Always mounted by `AppOverlays`; listens for `betterlectio:openOpgaveDetail` so any surface (incl. schedule deadline bricks) can open it without navigation.
...
- `components/ElevfeedbackSection.tsx` - Read-only Elevfeedback cards in the activity sheet/modal. Parses rendered `lectab=elevindhold` content and fails closed when a direct GET inherits edit mode: Lectio's empty textarea, `_backupContentDiv_`, and `_lv_` validator are editor infrastructure, never saved content. The presence of an editor textarea still counts as writable, and the UI directs the user to the native editor when committed content is unavailable in the static response. Requests are generation-guarded so switching activities cannot briefly show the previous activity's private notes. **Skriv/Rediger** opens `ElevfeedbackEditorOverlay` rather than mounting CKEditor in the 560px sheet. Student headings prefer Supabase `students.name`; Lectio's mixed-case `data-lectioContextCard` attribute is matched case-insensitively.
- `components/ElevfeedbackEditorOverlay.tsx` - Same-origin iframe of Lectio's LC/CKEditor Elevfeedback page (`name=bl-elevfeedback-editor`). Hides master menu, page header/subnav, entity nav, tabs, and TOC before first paint; relocates **Nyt** into the editor island while preserving native validation alerts/dialogs. Auto-enters **Rediger**, classifies edit/view/login/error documents, bounds loading with retry/direct-Lectio recovery, applies CKEditor dark mode, and protects dirty close via `LCDocumentEditor.ConfirmEditorDirtyAndNotSaving` with hidden-field fallback. Do not reimplement `LCDocumentEditor`, inject isolated HTML via srcdoc, or save through the Beskeder WYSIWYG.
...
- `components/settings/HoldMappingEditor.tsx` - Canonical lesson-key editor for subject names/colors (e.g. `1x MA`/`L2d MA`/`2zq MA`/`S2x MA`/`IB1 MA`/`BShannon DA` -> `ma`/`da`). Each subject also has an eye control backed by synced `assignments.hiddenSubjectKeys`; hiding removes that subject's assignments, filters, and counts from Opgaver, which links back to this settings section whenever subjects are hidden.
...
- `components/MobileAppInvitePopup.tsx` - Centered cross-platform app invite for the same automatic-promotion audience as `MobileAppDrawer`. Automatic display requires extension use on two distinct local calendar days, then snoozes for 7 days if untouched. It is suppressed during quiet hours (02:00–09:00), while in a class (uses `getCountdownState`), and during the first 24h after `students.extension_installed_at`. Snooze and bounded distinct-day state are per student in localStorage. The QR encodes `?u={studentId}` so a real scan stamps the first `app_qr_scanned_at` server-side and redirects to App Store or Google Play from the visitor's platform. Realtime exists only while its QR dialog is visible; after the channel joins, it reconciles the exact own row so a scan during subscription setup is not missed. It drives the one-time "Tak" transition and removes automatic promotion when `app_installed_at` appears. The sidebar/horizontal-navbar action remains available to every identified student and force-opens the QR even after install or opt-out. Funnel events are explicit and unsampled. Mounted as sibling to `<MobileAppDrawer />`. Helpers in `lib/mobile-app.ts`.
...
- `lib/opgave-detail.ts` - Fetch/parse ElevAflevering.aspx and perform assignment/group postbacks. Student feedback and submission history are mapped from Lectio's labels rather than fixed cell indexes: the history grid includes a duplicated mobile-only cell, and assignment types can vary student columns. Metadata parsing removes duplicated `.OnlyMobile` labels and normalizes Lectio's `timer` suffix before localized rendering; returned filenames repair Lectio's occasional UTF-8-as-Latin-1 Danish characters; `isMeaningfulAssignmentGrade` rejects Lectio's dash placeholders without rejecting the real `-3` grade. The requested `elevid` selects the correct feedback/status row and identifies the signed-in member for group assignments; do not use the table's `heavy` row, which can identify another primary/submitting member. Text-only teacher returns and multiline notes are displayed even without a grade. Group-member delete postbacks may live in `onclick` while `href="#"`; inspect both attributes and use the one containing `__doPostBack`. Empty/disabled group dropdown options are ignored, an exhausted group editor remains identifiable, and add/remove responses are verified by an actual membership change before the UI reports success.
- `lib/activity-detail.ts` - Fetch/parse `aktivitetforside2.aspx` with rich lektie content, presentation sections, øvrigt indhold, Elevfeedback tab ref (`lectab=elevindhold`), navigation/form tokens. `parseArticle` detects two special homework shapes: a heading wrapping a single `<a>` becomes `primaryLink` (clickable title, no duplicate pill); a body containing a single `<img>` becomes `image` (constrained click-to-enlarge preview). Webvisning anchors are converted from recognized `window.open`/`OpenWindow` commands into safe HTTP(S) anchors before all event handlers are stripped, and stay inline instead of gaining a duplicate pill. Elevfeedback view HTML is fetched separately — do not reuse `ensureActivityDoc()` (no `actHeader` on that tab).
- `lib/group-admin.ts` - Safe same-origin Lectio URL parsing for the conditional members-page `Rediger gruppe` action and the forside Hold og grupper links.
- `lib/elevfeedback.ts` - Parse/fetch Elevfeedback page and editor events (`openElevfeedbackEditor` / `notifyElevfeedbackUpdated`). A direct `lectab=elevindhold` GET can inherit edit mode, where committed content is not reliably present in the static response. Treat the editor textarea as write access but never render hidden `_backupContentDiv_` or `_lv_` alert/validator nodes as content; return `contentUnavailable` and fail closed instead. Save stays Lectio's CKEditor postback; do not round-trip LC HTML through the Beskeder BBCode editor.
- `lib/elevfeedback-frame.ts` - Idempotent chrome-strip CSS/helpers and document-state classification for the editor iframe (master menu, `#s_m_HeaderContent_subnav_div`, entity nav, tabs, TOC). Relocates **Nyt** out of the hidden TOC, then hides unrelated siblings of `.ls-texteditor-container` up to `<html>` without hiding Lectio/CKEditor alerts. Used by `elevfeedback-frame.content.ts` at `document_start`; its bootstrap observer must stop after initial load and must never rewrite its own style on every mutation.
...
- `lib/hold-mapping.ts` - Canonical lesson-key normalization (`1x MA`/`2.4 MA`/`L2d MA`/`2zq MA`/`S2x MA`/`IB1 MA`/`BShannon DA`/`buEUX2601/b DA` -> `ma`/`da`), shared mappings, ignored non-academic groups, legacy migration helpers. `getSubjectIdentityKey` is the shared identity for subject-level filters: recognized holds group across class years, while unknown holds use a collision-safe normalized raw fallback.
- `lib/hold-mapping-sync.ts` - Supabase v2 hydration + upsert/reset sync bridge for canonical mappings and user overrides
- `lib/settings-sync.ts` - Cross-device sync bridge for `bl-feature-settings` and per-school theme. Hydrates at bootstrap and foreground return with a persisted per-user five-minute throttle; due hydrations bypass the local query cache. It debounces pushes by 500ms and resolves conflicts last-writer-wins through security-definer RPCs. There is no settings/theme Realtime subscription or polling. Hydrate writes wrapped in `withSyncSuppressed()` prevent echo. When hydrate replaces local, `applySettingsSideEffects(prev, next)` re-applies live DOM/event effects, dispatches `betterlectio:settings-hydrated`, shows reload toast for `SETTINGS_REQUIRING_RELOAD`. Unauthorized errors suppressed from PostHog. Schema: `supabase/migrations/20260429_add_user_settings_sync.sql`.
...
- `lib/class-name.ts` - Shared class-name helpers for year->grade transforms and matching class codes: grade-based (`1x`, `2hf`, `2zq`, `1.4`), chained dotted (`10.st.kl.2`), prefixed (`L2d`, `S2x`, `IB1`), hyphenated (`3hx-u`), slash-suffixed vocational classes (`buEUX2601/b`), and named classes without a grade digit (`BShannon`, `Epsilon`). Normalizes Lectio hold IDs like `t25htxvx_1vx` to the class portion after the last `_`.
...
**Efficient event policy:** `lib/posthog.ts` allowlists cross-platform `app_active` and `app_load_completed` health signals, emitted once per 30-minute inactivity-defined session, plus canonical snake_case auth outcomes, feedback, onboarding, install/update, bypass, successful app-invite, and `referral_shared`. This preserves accurate DAU and approximate uses/day without a navigation firehose. `load_completed` measures each page at most once per browser session for a stable 10% user cohort; failures are unsampled. Once-per-session `feature_used` uses the same low-volume cohort policy. `identifyIfNeeded` creates/updates the canonical `lectio:<studentId>` person only when identity properties change, using a versioned cross-tab cache; person records carry `student_id`, string `school_id`, `last_platform`, and a durable `uses_extension = true` flag. Event-level `platform` is the source of truth for platform breakdowns. The referral edge functions send only successful attribution; clicks, rejections, and unlocks are measured from Postgres instead.

**Exceptions:** Explicit operational `$exception` callsites are retained. Transient network failures and invalidated extension-context errors are rejected centrally before any exception capture. Noisy ambient sources (`window.error`, unhandled rejections, `console.error`, background globals, and CSP violations) are deterministically sampled at 10%. All errors are deduplicated by source/message and capped at five per extension context, preventing storms from consuming the quota. The intentional bypass exception uses a stable fingerprint so releases do not create duplicate issues.
...
**Operational dashboard contract:** `docs/posthog-observability.md` defines the shared events, sampling interpretation, dashboard insights, and starter alert thresholds for extension/iOS/Android. Keep native event/property changes aligned with it.
...
**Edge function:** `supabase/functions/verify-lectio-auth/index.ts` handles QR-code-based auth. Flow: QR login → extract session cookies → sequential `SkemaNy.aspx` and `digitaltStudiekort.aspx` requests through the shared rotating jar → generate magic link → upsert student metadata. It never downloads or uploads the ordinary Lectio portrait. Schedule title is the fallback name source; missing profile enrichment must not block authentication.
...
**Retired generic audit log:** `public.updates` and its `lessons`/`homework_entries` audit triggers are removed by `20260826_remove_unused_updates_audit.sql`; they had no reader and created substantial write/storage churn. Keep `admin_audit_log` and the separate lesson notification trigger intact.

**Realtime publication:** `supabase_realtime` remains selective. `20260827_cleanup_supabase_realtime_publication.sql` keeps only `public.students` published for these flows: settings hydrate by refetch, homework fresh-loads/polls while visible, and QR Realtime is scoped to an open QR dialog with post-join reconciliation.

**Profile picture storage:** Ordinary school portraits remain on Lectio and are requested with the viewer's authenticated Lectio session; clients cache the parsed `pictureid` for 24 hours with stale-while-refresh. Never download or copy these portraits into Supabase. The public `profile-pictures` bucket is reserved for approved custom images under immutable `custom/{schoolId}/{studentId}/{submissionId}.jpg` paths. Referral-unlocked students may submit JPEG/PNG/WebP images (5MB, 25MP, 8000px/side maximum) through `profile-picture-submit`; submissions live privately and never affect rendering until approval. Admin must decode through `normalizeProfilePicture`, which strips metadata and publishes only a bounded 1600px sRGB JPEG; never render or publish the original object. Rejections carry a required reason and allow immediate retry. Cleanup is performed by `maintenance-cleanup`. `students.lectio_pfp_url` and `pfp_hash` remain deprecated null-only compatibility fields until old clients can no longer query them. Schema/RPCs: `20260803_add_moderated_profile_pictures.sql`, `20260805_allow_ios_profile_picture_submissions.sql`, `20260826_stop_mirroring_lectio_profile_pictures.sql`; Admin queue: `admin/app/(dashboard)/moderation`.

**Student identity rendering rule:** When a UI surface can identify a student (`students.id`, raw `elevid`, or lookup ID like `S727...`), prefer `students.name` for display, then keep Lectio names as aliases/search terms. For pictures: approved `custom_pfp_url` → authenticated Lectio/context-card image fetch → initials; a custom-image load failure must continue to the Lectio candidate rather than jumping straight to initials. Never render the deprecated `lectio_pfp_url`. Applies to FindSkema, members, Beskeder, group submissions, sidebar.
...
**Referral system:** Classmates share `https://betterlectio.dk/r/{referrer_elevid}`. `referral-click` ignores known link-preview/calendar crawlers and creates rate-limited, daily-IP-hashed click tokens for real visits; supplied tokens must be validated against the referrer before clients persist them. iOS App Clip creates the token itself when Apple invokes the original tokenless URL and shares it with the full app through the dedicated App Group. `referral-finalize` verifies JWT ownership and delegates to `finalize_referral_attribution`, whose row lock and single database transaction make the click conversion, student attribution, and three-conversion reward atomic. Extension finalization is awaited during auth so the MV3 worker stays alive; any network/5xx/kill-switch failure is retried from later ensure-session calls until a parsed 2xx outcome is stored locally. Fresh-install window is 7 days; click retention is 180 days. `REFERRALS_ENABLED=false` is the server kill switch. Schema: `20260430_add_referral_tracking.sql`, `20260723_referral_reward_unlocked_at.sql`, `20260807_atomic_referral_finalization.sql`. Admin: `admin/app/(dashboard)/referrals/page.tsx`.
...
Feedback threads keep product status separate from conversation state (`awaiting_staff`, `awaiting_user`, `resolved`). `list_my_feedback`, `get_my_feedback_thread`, `reply_to_feedback`, and `mark_feedback_read` are the cross-platform user contract; `admin_reply_to_feedback` is service-role-only. Public comments are visible to the ticket owner, while `is_internal` comments remain admin-only. Schema: `supabase/migrations/20260918090000_expand_feedback_threads.sql`.
...
bun run lectio session import --student ID --school ID # Import consented Supabase jar
