...
**Efficient event policy:** `lib/posthog.ts` allowlists cross-platform `app_active` and `app_load_completed` health signals, emitted once per 30-minute inactivity-defined session, plus feedback, onboarding, install/update, bypass, successful app-invite, and referral-share outcomes. This preserves accurate DAU and approximate uses/day without a navigation firehose. Once-per-session `feature used` remains on a stable 10% monthly user cohort. `identifyIfNeeded` creates/updates the canonical person only when its identity properties change, using a cross-tab local cache. The referral edge functions send only successful `referral attributed`; clicks, rejections, and unlocks are measured from Postgres instead.
