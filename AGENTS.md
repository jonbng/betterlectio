...
- `components/ReferralShareCard.tsx` - "Inviter klassekammerater" share card in Settings → Inviter. Shows `betterlectio.dk/r/{elevid}` link with copy + native share, one-click Lectio invitations, plus stats (clicks, conversions) and recent attributed classmates. Stats from `get_referral_stats(student_id)` RPC. Fires `referral share link copied` on copy/share.
