...
### Components
- `components/FeedbackWidget.tsx` + `components/FeedbackInbox.tsx` - Shared feedback entry point. It preserves screenshot-assisted submission and exposes the authenticated user's cross-platform request inbox, thread history, unread replies, and reply composer. Reads and writes use the ownership-checking feedback RPCs.
...
- `lib/opgave-detail.ts` - Fetch/parse ElevAflevering.aspx, submission API, localStorage cache. Group-member delete postbacks may live in `onclick` while `href="#"`; inspect both attributes and use the one containing `__doPostBack`.
...
- `lib/hold-mapping.ts` - Canonical lesson-key normalization (`1x MA`/`2.4 MA`/`L2d MA`/`2zq MA`/`S2x MA`/`IB1 MA`/`BShannon DA`/`buEUX2601/b DA` -> `ma`/`da`), shared mappings, ignored non-academic groups, legacy migration helpers
