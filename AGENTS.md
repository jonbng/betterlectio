...
- `components/ActivityClassModal.tsx` - Activity detail modal from skema/forside links, now rendering lektier, presentation content, øvrigt indhold, and related links in the side sheet. Image-only homework items render as constrained click-to-zoom previews; clicking image previews, link-only homework titles, or any homework link whose extension is an image (`jpg/png/gif/webp/svg/heic/...`) or `pdf` opens the shared `<Lightbox>` overlay (same UX as `BeskederThreadView`'s attachment lightbox) instead of navigating to Lectio.
- `components/Lightbox.tsx` - Shared overlay viewer for previewing images and PDFs with a darkened backdrop, click-outside / Esc to close, and a download/close footer. Exports `LightboxItem`, plus `extensionFromUrlOrName()` and `lightboxKindForExtension()` helpers used by callers to decide whether a URL/file can be lightboxed. PDFs are fetched as blobs (`credentials: 'include'`) so Lectio's `Content-Disposition: attachment` doesn't force a download. Uses the existing `lightbox-fade-in` / `lightbox-scale-in` keyframes from `globals.css`.
...
- `components/MobileAppDrawer.tsx` - Bottom-right floating drawer that pitches the iOS BetterLectio app to eligible students. Visible only when `students.app_eligible=true && app_installed_at is null && app_qr_scanned_at is null && marked_android_at is null && dismissed_app_prompt_at is null`. Collapsed = small `Smartphone` button; expanded = horizontal panel with QR code (via `qrcode` lib) pointing to `https://betterlectio.dk/download/ios?u={studentId}`, plus a single "Jeg er på Android" CTA that writes `marked_android_at` so the prompt is permanently suppressed. `app_installed_at` is set server-side when a student logs into the mobile app, and `app_qr_scanned_at` is set when they scan the QR — both hide the surface automatically. Mounted in `entrypoints/content.tsx`'s `DashboardLayout` sibling to `<Toaster />`. Shares `appStoreUrlFor()` + `renderAppStoreQrSvg()` with the invite popup via `lib/mobile-app.ts`.
- `components/MobileAppInvitePopup.tsx` - Centered "early access" invite dialog for the same eligible students as `MobileAppDrawer` (identical eligibility fields, including `app_qr_scanned_at`). Shown once on page load, then re-prompted **7 days later** if the student still hasn't acted on it. Suppressed during quiet hours (02:00–09:00 local), while the student is currently in a class (`getCountdownState` from `ScheduleCountdown` against `getCachedSchedule`/`getTodaySchedule`), and during the **first 24 hours** after `students.extension_installed_at` (so it never competes with the day-0 onboarding popup, even on a second browser — the gate is cross-device because the timestamp is set on first-ever Supabase auth). Snooze timestamp is stored in localStorage at `bl-mobile-app-invite-last-shown:{studentId}` (per-device, no migration). The QR encodes `https://betterlectio.dk/download/ios?u={studentId}` so a real scan stamps `students.app_qr_scanned_at` server-side. The popup subscribes to a Realtime channel on the `students` row (`mobile-app-invite:{schoolId}:{studentId}`); when `app_qr_scanned_at` flips while the popup is open, the content cross-fades (with a blur mask) to a "Tak / Thanks for installing" success view (Check icon + halo ripple), then auto-closes after **4 seconds**. A `hasOpenedOnce` lock keeps the popup mounted across the row update so the success animation can play instead of getting unmounted by the parent's eligibility gate. The success transition fires **at most once per student** (gated on `bl-mobile-app-invite-thanks-shown:{studentId}` in localStorage, set by `markInviteThanksShown` in `lib/mobile-app.ts`); subsequent opens — e.g. via the sidebar debug trigger after scanning — keep showing the QR / invite content. The X button, Esc, and outside-click also close the popup during the success state (interrupting the auto-close timer), so the user is never trapped in the thanks view. Layered shadows (3-stop), strong custom easings (`cubic-bezier(0.23, 1, 0.32, 1)` and `cubic-bezier(0.32, 0.72, 0, 1)`), `:active scale(0.97)` on buttons, staggered entrance (eyebrow→title→body→button at 20/70/120/170ms), `text-balance`/`text-pretty` on copy, and `prefers-reduced-motion` support. There is **no** "I'll scan" CTA — only a soft close (X / outside-click / Esc) and a single "Jeg er på Android" action that writes `marked_android_at`. Keyframes (`bl-rise`, `bl-qr-in`, `bl-success-pop`, `bl-success-halo`) live in `styles/globals.css`. Mounted as a sibling to `<MobileAppDrawer />` in `DashboardLayout`. Helpers live in `lib/mobile-app.ts` (`appStoreUrlFor`, `isInviteSnoozed`, `stampInviteShown`, `getInviteSnoozeAt`).
...
- `lib/activity-detail.ts` - Fetch/parse aktivitetforside2.aspx with rich lektie content, presentation sections, øvrigt indhold, navigation/form tokens + cache. `parseArticle` detects two special homework shapes: (1) when the heading wraps a single `<a>` (e.g. `<h1><a href="...Tornerose.pdf">Tornerose.pdf</a></h1>`) it emits `primaryLink` and removes that link from `links` so the renderer can promote the title to a clickable link without duplicating it as a pill below; (2) when the body contains a single `<img>` and nothing else of substance, it emits `image` (and pulls the img out of `contentHtml`) so the renderer can show a constrained, click-to-enlarge preview instead of an unbounded raw image with explicit `width`/`height` attributes.
...
- `lib/settings-sync.ts` - Cross-device sync bridge for `bl-feature-settings` (single jsonb blob) and per-school theme (`bl-school-themes-v1`). Hydrates on bootstrap, debounced push (500ms) on every `saveSettings` / `saveThemePreferenceForSchool`, last-writer-wins via `upsert_user_settings(p_settings, p_client_updated_at, p_schema_version)` and `upsert_user_school_theme(p_school_id, p_theme_id, p_client_updated_at)`. Realtime subscription on both `user_settings` and `user_school_themes` filtered by `supabase_id=eq.<auth uid>` re-hydrates when another device writes. Hydrate writes are wrapped in `withSyncSuppressed()` so the resulting `saveSettings` doesn't echo back as a push. When hydrate replaces local, `applySettingsSideEffects(prev, next)` re-applies live DOM/event side effects (dark mode, locale, opgave deadlines, opt-out mirror), dispatches `betterlectio:settings-hydrated` so the sidebar re-reads, and shows the same Sonner reload toast as `handleSettingChange` for `SETTINGS_REQUIRING_RELOAD`. Unauthorized errors are suppressed from PostHog (same pattern as hold-mapping-sync). Schema lives in `supabase/migrations/20260429_add_user_settings_sync.sql`.
...
- `lib/opgaver-deadlines-cache.ts` - School-scoped localStorage cache of the parsed `OpgaveEntry[]` list (6h TTL). Populated by `OpgaverPage.tsx` whenever it has fresh data, and refreshed in the background by the schedule page on load via `fetchAndCacheOpgaver` (fetches `OpgaverElev.aspx` and toggles `CurrentExerciseFilterCB`/`ShowThisTermOnlyCB` if filtered). Read by `injectDeadlineBricks()` in `entrypoints/content.tsx` to render assignment deadline bricks at the deadline's exact time on the corresponding `td[data-date]` cell. Only renders on own schedule (`isViewingOwnPage()` true), only on `skemany.aspx` / `skema1dag.aspx`, and only when the new `schedule.opgaveDeadlines` setting is on (default true). Submitted assignments (`status === 'afleveret'`) are filtered out; `mangler` past deadlines stay rendered. Click on a brick dispatches `betterlectio:openOpgaveDetail` (caught by `AppSidebar`'s mounted `OpgaveDetailSheet`).
...
- `mobile_app_invite_success_shown` — fires when the popup transitions to its "Tak for installation" success view because Realtime saw `app_qr_scanned_at` flip while the popup was open. Props: `school_id`.
- `mobile_app_invite_opened_from_drawer` — drawer's "Læs mere" button opened the full invite popup. Props: `school_id`.
- `mobile_app_marked_android` — extended to include `source: 'invite_popup' | 'tab'` (originally only fired from the drawer). Writes `marked_android_at` on the student row, hiding both surfaces permanently.
