...
- `components/ElevfeedbackSection.tsx` - Read-only Elevfeedback cards in the activity sheet/modal. Parses `lectab=elevindhold` HTML even when Lectio inherits edit mode: edit-mode saved text is recovered from LC's hidden `_backupContentDiv_` / `_lv_` nodes, and the presence of an editor textarea counts as writable. Requests are generation-guarded so switching activities cannot briefly show the previous activity's private notes. **Skriv/Rediger** opens `ElevfeedbackEditorOverlay` rather than mounting CKEditor in the 560px sheet. Student headings prefer Supabase `students.name`; Lectio's mixed-case `data-lectioContextCard` attribute is matched case-insensitively.
...
- `lib/elevfeedback.ts` - Parse/fetch Elevfeedback page and editor events (`openElevfeedbackEditor` / `notifyElevfeedbackUpdated`). A direct `lectab=elevindhold` GET can inherit edit mode, where the textarea is empty before JavaScript runs and saved text lives in hidden LC fallback nodes; preserve that fallback for read-only rendering and treat the editor textarea as write access. Save stays Lectio's CKEditor postback; do not round-trip LC HTML through the Beskeder BBCode editor.
