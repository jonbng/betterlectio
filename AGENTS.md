...
- `components/AppOverlays.tsx` - Layout-independent owner for Settings, onboarding, activity/private-appointment dialogs, and the assignment detail sheet. Keep these outside either navigation surface so global events work in both layouts.
- `components/OnboardingWizard.tsx` - First-run setup for theme, navigation layout, subject colors, and optional profile details. Navigation asks users to choose between the default/recommended sidebar and the Lectio-like top menu; finishing reloads only when the mounted navigation shell must change.
