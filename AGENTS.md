...
- `components/OpgaveDetailSheet.tsx` - Assignment detail side sheet with submission history, comment/file upload, and Supabase-first group-member names/avatars. Always mounted in `AppSidebar`; listens for `betterlectio:openOpgaveDetail` event so any surface (incl. schedule deadline bricks) can open it without navigation.
- `components/BeskederThreadView.tsx` - Thread view with Supabase-first sender names/avatars, WYSIWYG reply, no-reload reply/attach
...
- `components/ActivityClassModal.tsx` - Activity detail modal from skema/forside, rendering lektier, presentation, øvrigt indhold, links. Image-only homework items render as click-to-zoom previews; image previews, link-only homework titles, and homework links with image/`pdf` extensions open the shared `<Lightbox>` instead of navigating.
...
- `components/ForsideDashboard.tsx` - Redesigned forside: 4 cards (aktuel info, lektier, opgaver, beskeder) parsed from native DOM, 2-col grid with priority indicators, hold colors, urgency bars, relative times, Supabase-first names/avatars in message previews. Other native dashboard islands (e.g. Registreringer) parsed via `parseGenericIslands()` and rendered through `GenericCard` with consistent styling, alternated across columns.
- `components/ForsideOpgaverCard.tsx` - Forside opgaver card with urgency design (parser reused by ForsideDashboard)
- `components/KaraktererPage.tsx` - Grade report redesign: subject cards with big color-coded grades, teacher notes inline, summary bar, collapsible diploma/protocol/remarks, DOM parser
...
- `components/MobileAppDrawer.tsx` - Bottom-right floating drawer pitching the iOS app to eligible students. Visible only when `students.app_eligible=true && app_installed_at is null && app_qr_scanned_at is null && marked_android_at is null && dismissed_app_prompt_at is null`. Collapsed = small button; expanded = horizontal panel with QR code pointing to `https://betterlectio.dk/download/ios?u={studentId}`, plus "Jeg er på Android" CTA writing `marked_android_at`. Mounted in `DashboardLayout`. Helpers in `lib/mobile-app.ts`.
- `components/ReferralShareCard.tsx` - "Inviter klassekammerater" share card in Settings → Inviter. Shows `betterlectio.dk/r/{elevid}` link with copy + native share, plus stats (clicks, conversions) and recent attributed classmates. Stats from `get_referral_stats(student_id)` RPC. Fires `referral share link copied` on copy/share.
- `components/MobileAppInvitePopup.tsx` - Centered "early access" invite dialog for the same eligible students as `MobileAppDrawer`. Shown once on page load, then 7 days later if untouched. Suppressed during quiet hours (02:00–09:00), while in a class (uses `getCountdownState`), and during the first 24h after `students.extension_installed_at` (so it doesn't compete with the day-0 onboarding popup). Snooze stamp at `bl-mobile-app-invite-last-shown:{studentId}`. The QR encodes `?u={studentId}` so a real scan stamps `app_qr_scanned_at` server-side. Subscribes to Realtime on the `students` row; when `app_qr_scanned_at` flips while open, cross-fades to a "Tak" success view (auto-closes after 4s). Success transition fires at most once per student (gated on `bl-mobile-app-invite-thanks-shown:{studentId}`). No "I'll scan" CTA — only soft close and "Jeg er på Android". Mounted as sibling to `<MobileAppDrawer />`. Helpers in `lib/mobile-app.ts`.
...
- `lib/activity-detail.ts` - Fetch/parse aktivitetforside2.aspx with rich lektie content, presentation sections, øvrigt indhold, navigation/form tokens + cache. `parseArticle` detects two special homework shapes: a heading wrapping a single `<a>` becomes `primaryLink` (clickable title, no duplicate pill); a body containing a single `<img>` becomes `image` (constrained click-to-enlarge preview).
...
- `lib/hold-mapping.ts` - Canonical lesson-key normalization (`1x MA`/`2.4 MA`/`L2d MA`/`2zq MA`/`S2x MA`/`IB1 MA` -> `ma`), shared mappings, ignored non-academic groups, legacy migration helpers
...
- `lib/mobile-app.ts` - Shared `appStoreUrlFor(studentId)` + `renderAppStoreQrSvg(studentId)` helpers used by both `MobileAppDrawer` and `MobileAppInvitePopup`. studentId appended as `?u=` so `/download/ios` redirect can stamp `app_qr_scanned_at` server-side. Owns per-student invite snooze in localStorage (`isInviteSnoozed`, `stampInviteShown`, `getInviteSnoozeAt`) — 7-day cooldown.
...
**Identify:** On each page load, `identifyIfNeeded()` sets person properties: `name`, `school_id`, `school_name`, `class_name`, `school_year`, `dark_mode`, `theme_id`, `extension_version`, `lectio_version`. PostHog auto-wraps as `$set` — never wrap manually. Use `setPersonProperties()` for targeted profile updates.

**Background analytics rule:** Never call `identify()` from `entrypoints/background.ts` with partial properties. Background events must first resolve a named student row from Supabase; if `students.name` is missing, skip the event rather than creating a nameless person.

**Events:**
- `extension loaded` — DAU, school, page
- `extension installed` / `extension updated` — queued in background, emitted once user is identified
- `supabase auth succeeded/failed` — include `source` (`bootstrap`, `hold-mapping-sync`, etc.) so first-attempt failures can be traced. Preserve `auth_stage` and `auth_server_school_id` when edge function provides them.
- `setting changed` / `theme changed`
- `betterlectio profile updated` — own profile edits (`description`, `instagram`, `show_birthday`)
...
- `mobile_app_invite_shown` / `mobile_app_invite_dismissed` / `mobile_app_invite_success_shown` / `mobile_app_invite_opened_from_drawer` / `mobile_app_marked_android` — iOS app invite lifecycle
- `lectio session lost` — passive logout detection
- `lectio native error` — Lectio's native error popup fired, usually meaning the extension broke a postback. Props include `error_title`, `error_body`, `dialog_html`, `recent_urls`, `previous_url`. Paired with `captureException`. User sees Sonner toast confirming report.
- `betterlectio bypass engaged` — user pressed sidebar "vis original Lectio" escape hatch. Strong signal a redesign is broken. Rich props: identity, page context, settings, display, any visible Lectio error popup. Paired `captureException` with `source: 'bypass_button'`. Flushed synchronously before reload (1500ms cap). Bypass stays armed for 5 minutes (auto-expiry) or until the user clicks the floating re-enable button injected by `content.tsx` (`injectBypassReenableButton`); state lives in localStorage at `bl-bypass-redesigns` so it shares across tabs. Helpers: `armBypass`, `disableBypass`, `isBypassActive`, `getBypassRemainingMs` in `lib/bypass-redesigns.ts`.
- `settings synced from cloud` / `settings sync failed`
- `referral link clicked` / `referral link clicked invalid` / `referral attributed` / `referral attribution rejected` / `referral share link copied`
- `captureException` — sent via posthog-node `$exception` pipeline. In content scripts, `distinctId` can be omitted: `getContentDistinctId()` resolves from `__IL_CACHED_PROFILE__`/`getCachedProfile()`. Each exception merges auto props + `recent_urls`, `referrer`, `student_id`, `class_name`, `runtime`. Global handlers tag `source` (`window.error`, `unhandledrejection`, `console.error` — throttled), `background`, `background-unhandledrejection`. Rate limit: 25/page in content script.

**Adding new events:** Be conservative — we're on PostHog's free tier. Only capture when you have an identified user. Prefer `captureFeatureUsedOncePerSession()` for feature-adoption telemetry and `identifyIfNeeded()` / `setPersonProperties()` for person updates. All calls are try/catch wrapped.

**Auto properties:** Every `capture()` includes `$browser`, `$os`, `$screen_height`, `$screen_width`, `$current_url`, `$pathname`, `extension_version`. `captureException()` merges the same set in page contexts.
...
**Profile picture storage:** Pictures downloaded from Lectio (using session cookies) and uploaded to `profile-pictures` bucket at `{schoolId}/{userId}.{ext}`. Public URL stored in `students.custom_pfp_url`. Original Lectio URL kept in `lectio_pfp_url`. Bucket: public, jpeg/png/webp/gif, 5MB limit.
...
**Referral system:** Classmates share `https://betterlectio.dk/r/{referrer_elevid}`. The link 302s through `website/app/r/[elevid]/route.ts` to `referral-click` edge function, which inserts a `referral_clicks` row, sets a 180-day `bl_ref` cookie on `*.supabase.co` (`SameSite=None; Secure; HttpOnly`), and 302s on to `betterlectio.dk/download?ref=1`. After QR-auth, when `verify-lectio-auth` reports `wasFirstInstall: true`, background POSTs to `referral-finalize` with `credentials: 'include'`. That function reads the cookie, validates eligibility (no self-referral, no existing `referred_by`, fresh install ≤ 7d, click ≤ 180d), stamps `students.referred_by` + `referred_at` + `referral_click_id`, marks click row converted, clears cookie, returns referrer's name so content script shows toast. Attempts deduped by `bl-referral-finalize-attempted:{studentId}` flag. Eligible students see share card in **Settings → Inviter**, backed by `get_referral_stats(student_id)` RPC. Schema: `supabase/migrations/20260430_add_referral_tracking.sql`. Admin: `admin/app/(dashboard)/referrals/page.tsx`.
...
Next.js 16 App Router site at `betterlectio.dk`. Routes: `/download`, `/download/ios`, `/privatliv`, `/uninstall`, `/r/[elevid]` (referral redirect), and `/skoler/[slug]` (SEO).
