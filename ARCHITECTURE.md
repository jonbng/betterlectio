...
| `lib/posthog.ts` | Efficient PostHog boundary (edge build). Distinct ID: `lectio:${studentId}`; no anonymous IDs. High-value outcomes are allowlisted; `extension loaded` is captured at 100% (it is the bypass-rate denominator), only once-per-session `feature used` uses a stable 10% monthly cohort, and identify helpers are no-ops. `getPageSlug()` lowercases every analytics `page` value so mixed-case Lectio paths don't split one page into two. Explicit operational exceptions are retained; noisy globals are sampled at 10%, with all errors deduped/capped at five per context. Page-hide flushing occurs only after a client is created. |
