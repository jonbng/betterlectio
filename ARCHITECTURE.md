...
| `lib/posthog.ts` | Efficient PostHog boundary (edge build). Distinct ID: `lectio:${studentId}`; no anonymous IDs. `app_active` and `app_load_completed` use a 30-minute inactivity session window, preserving DAU and approximate uses/day without a navigation firehose; successful feature usage remains on a stable 10% monthly cohort. `identifyIfNeeded` updates the canonical person only when identity properties change, using a cross-tab cache. Explicit operational exceptions are retained; transient network/context-invalidated noise is rejected centrally, noisy globals are sampled at 10%, and all errors are deduped/capped at five per context. Page slugs are normalized to lowercase and the intentional bypass exception has a release-stable fingerprint. Page-hide flushing occurs only after a client is created. |
