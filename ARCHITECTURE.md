...
PostHog telemetry: `referral link clicked`, `referral link clicked invalid`, `referral attributed`, `referral attribution rejected` (server-side from edge functions), `referral share link copied` (client-side). Schema migration: `supabase/migrations/20260430_add_referral_tracking.sql`. Admin dashboard: `admin/app/(dashboard)/referrals/page.tsx` with `getReferralOverview / getReferralTimeSeries / getTopReferrers / getRecentAttributions / getReferralRejectionBreakdown / getReferralCountryBreakdown` in `admin/lib/supabase/queries.ts`. Both edge functions deploy via `bunx supabase functions deploy referral-click` (no JWT needed — public redirect endpoint) and `bunx supabase functions deploy referral-finalize` (JWT-required).
