...
5. Parse the current Lectio `pictureid`; ordinary portraits remain on Lectio and are loaded by clients with their existing Lectio session
...
**Explicitly consented session donors:** The unified auth function keeps its otherwise-temporary full rotating jar only when the Lectio-resolved `(student_id, school_id)` matches an active `lectio_session_grants` row. The allowlist, credentials, audit table, and all RPCs are service-role-only with RLS and explicit revokes. Each jar is encrypted before database storage with a random AES-256-GCM data key; that key is wrapped by a versioned master key and both layers bind the student/school identity as authenticated additional data. Revocation synchronously deletes the credential. `lectio-session-keepalive` leases due rows with `FOR UPDATE SKIP LOCKED`, requests `/ping.aspx`, follows redirects only within HTTPS `www.lectio.dk`, saves cookie rotation, retries transient failures, and disables dead sessions. Supabase Cron invokes it every 30 minutes using a bearer secret and project URL read from Vault. The CLI can export an active grant through an audited RPC and decrypt it locally using environment-only service-role/master-key credentials; cookie files are mode `0600`. See `docs/lectio-session-operations.md`.

**Admin browser session handoff:** The privileged `admin-handoff.content.ts` bridge is a compile-time-only WXT entrypoint. `BETTERLECTIO_ADMIN_BUILD=true` enables it, adds `cookies` plus Lectio/dashboard host permissions, assigns a separate Firefox ID/name, and redirects output to `.output-admin`; production discovery removes the bridge before bundling and retains its original manifest. After an authenticated admin clicks **Log in as user**, the server stores only the SHA-256 hash of a random 60-second token. The dashboard posts the raw token to the bridge, whose background worker redeems it once at `/api/extension/lectio-session-handoff`. The service-role-only consume RPC atomically marks it used, verifies the grant/credential is active, and records access before the server decrypts and returns the jar with `no-store`. Before installing it, the admin extension signs out its previous Supabase identity, clears isolated local state, and replaces only the current browser cookie store's Lectio cookies. Ordinary builds contain no bridge code, admin endpoint string, cookie permission, or Lectio/dashboard host permission. Never expose service-role or master-key material through `VITE_*` variables.

**Admin native session import:** Dedicated development-only iOS and Android application variants authenticate to `admin.betterlectio.dk` with a rotatable mobile-only bearer and request only active, explicitly consented jars. Decryption remains server-side and export uses the audited `export_lectio_session_credential` RPC. After native identity validation, the clients invoke universal `lectio-auth` as `admin-ios` / `admin-android`; this refreshes the encrypted rotating jar and creates the owned Supabase session without stamping ordinary app installation fields. Normal app artifacts contain neither the bearer nor the admin API client.
...
**Storage bucket** `profile-pictures`: public and reserved for approved, normalized custom portraits under immutable `custom/{schoolId}/{studentId}/{submissionId}.jpg` paths. Ordinary Lectio portraits are never copied here.
...
**Database write hygiene:** The unused generic `public.updates` audit table and only its `lessons`/`homework_entries` writer triggers are retired by `20260826_remove_unused_updates_audit.sql`. The migration fails closed on unexpected sources or dependencies and does not affect `admin_audit_log` or the separate lesson-change notification trigger.
...
**Settings sync:** User settings (`bl-feature-settings`) and per-school theme (`bl-school-themes-v1`) are synced to Supabase. Two tables keyed on `auth.uid()`: `user_settings` (single jsonb blob) and `user_school_themes` (one row per school). Writes go through security-definer RPCs (`upsert_user_settings`, `upsert_user_school_theme`) enforcing last-writer-wins via client clock. Hydrate at bootstrap and foreground return with a persisted per-user five-minute throttle; due hydrations bypass the local query cache. If remote is newer, local is replaced and `applySettingsSideEffects(prev, next)` re-applies live DOM/event effects (dark mode, locale, opgave deadlines event, opt-out mirror). `betterlectio:settings-hydrated` event re-renders sidebar; `SETTINGS_REQUIRING_RELOAD` shows reload toast. Settings/theme use neither a Realtime subscription nor background polling. All client writes route through `saveSettings` and `saveThemePreferenceForSchool`; `withSyncSuppressed()` depth counter prevents hydrate writes from echoing back. Schema: `supabase/migrations/20260429_add_user_settings_sync.sql`.
...
2. `referral-click` validates the referrer, ignores known link-preview/calendar crawlers, enforces per-IP/per-referrer limits, inserts metadata (UA, Referer, daily-rotated IP hash, coarse location), sets a 180-day `bl_ref` cookie for extension attribution, and redirects to the appropriate destination. Pre-supplied native tokens are validated against an active click row before persistence.
3. After install, `runEnsureSupabaseSession` calls `maybeFinalizeReferral` once a usable owned session exists. The server remains the authority for the seven-day first-install eligibility window.
4. `maybeFinalizeReferral` POSTs with credentials, the publishable key, and the user JWT. It records its per-student completion flag only after a parsed 2xx outcome; network/5xx/kill-switch failures remain retryable. Auth awaits the request so the MV3 worker stays alive, and later ensure-session calls retry any non-definitive attempt.
...
| `FeedbackWidget.tsx` / `FeedbackInbox.tsx` | Floating submit surface plus authenticated cross-platform feedback inbox, thread detail, unread state, and user replies. Uses ownership-checking RPCs through the background Supabase proxy. |
...
| `MobileAppInvitePopup.tsx` | Centered cross-platform invite for the same automatic-promotion audience. Automatic display requires extension use on two distinct local calendar days, then snoozes for 7 days; it is also suppressed during the first 24h, quiet hours (02:00–09:00), and while in class. A first QR scan stamps `app_qr_scanned_at`, triggers the Realtime thank-you transition, and redirects by platform. Realtime exists only while the QR dialog is visible; once joined, the dialog refetches its exact student row to reconcile a scan that raced setup. The scan no longer suppresses future promotion, while `app_installed_at` suppresses automatic promotion. The navigation action stays available and force-opens the QR after install or opt-out. Funnel events are explicit and unsampled. |

**Feedback AI triage:** `20260918120000_add_feedback_ai_triage.sql` queues every new or materially edited ticket without delaying submission. Supabase Cron calls `feedback-ai-triage` once per minute; leased jobs retry with exponential backoff and remain safe under overlapping workers. Gemini produces a concise title, summary, severity, relevance classification, and tags. An absent human title/priority is filled automatically, while every AI value remains visible and overridable in Admin. `gemini-embedding-001` stores 768-dimensional vectors in pgvector; indexed nearest-neighbor retrieval narrows candidates before Gemini reranks likely duplicates. Suggestions require an explicit Admin accept/dismiss decision and are never auto-merged. `GEMINI_API_KEY` is required; `GEMINI_MODEL` defaults to `gemini-3.6-flash` (the current free-tier model returned by Gemini) and `GEMINI_EMBEDDING_MODEL` defaults to `gemini-embedding-001`. The scheduler prefers Vault secrets `feedback_ai_project_url` / `feedback_ai_cron_secret` and can reuse the existing keepalive scheduler pair. The bearer value must match `FEEDBACK_AI_CRON_SECRET` (or the existing `LECTIO_KEEPALIVE_CRON_SECRET`) in Edge Function secrets.
...
**Student identity resolution:** Prefer `students.name` for display, keep Lectio names as aliases/search terms. Pictures: approved `custom_pfp_url` → authenticated Lectio/context-card image → initials, including retrying the Lectio candidate when a custom object fails to load. Lectio picture IDs are cached locally for 24 hours with stale-while-refresh; `lectio_pfp_url` and `pfp_hash` are deprecated null-only compatibility columns. Helpers in `lib/supabase/student-lookup.ts` accept both raw `elevid` and prefixed lookup IDs (`S727...`) so message names/avatars, FindSkema, member grids, group submissions, sidebar/profile stay consistent.
...
**Homework completion refresh:** `LektierPage.tsx` fresh-loads completion state on mount and foreground return, then polls every 60 seconds only while visible, bypassing the local query cache. It does not use Realtime; cross-device changes converge on the next fresh load or visible poll.

**Realtime publication:** `20260827_cleanup_supabase_realtime_publication.sql` keeps the selective `supabase_realtime` publication limited to `public.students` for these flows. The QR dialog is its only remaining consumer; settings use bootstrap/foreground hydration and homework uses fresh-load/visible polling.
...
| `lib/posthog.ts` | Efficient PostHog boundary (edge build). Distinct ID: `lectio:${studentId}`; no anonymous IDs. Canonical snake_case event normalization prevents legacy callsites from fragmenting reports. `app_active` and `app_load_completed` use a 30-minute inactivity session window, preserving DAU and approximate uses/day without a navigation firehose. `load_completed` keeps one timing per page/session for a stable 10% user cohort while retaining all failures; feature usage has an equivalent sampled policy. `identifyIfNeeded` updates the canonical person only when identity properties change, using a versioned cross-tab cache; it sets shared identity keys and durable `uses_extension`. Explicit operational exceptions are retained; transient network/context-invalidated noise is rejected centrally, noisy globals are sampled at 10%, and all errors are deduped/capped at five per context. Page slugs are normalized to lowercase and the intentional bypass exception has a release-stable fingerprint. Page-hide flushing occurs only after a client is created. |
| `lib/posthog-lifecycle.ts` | Persists install/update lifecycle outcomes until an identified content-script session can emit canonical `extension_installed` / `extension_updated` events. |
| `docs/posthog-observability.md` | Cross-platform PostHog identity/event contract, low-volume sampling rules, dashboard panels, and incident alert thresholds. |
...
| `lib/lectio-error-popup.ts` | MutationObserver detector for Lectio's native error popup (`[data-title^="Fejl"]`). Extracts title + body, dedupes per element, and reports the actionable failure through the capped `captureException` path. |
...
**Privacy page (`app/privatliv/page.tsx`)** — rebuilt as an approachable, Danish, trust-and-reassurance page instead of a legal wall of text; it works as both privacy disclosure and marketing. Server component, no client JS. Flow: reassurance hero + trust chips → three promise pillars → a signature "honest ledger" (side-by-side *Det gør vi* / *Det gør vi aldrig*) → a dark card explaining that usernames/passwords are never collected and disclosing the narrow, explicit-consent encrypted session-cookie program → the three services (Supabase/PostHog/UserJot) each in one plain sentence with a "kun hvis…" tag → the complete legal detail hidden inside native `<details>`/`<summary>` accordions (accesses, external data, permissions, cookies/tracking, who's behind it) → an open-source + contact CTA. Every disclosure from the previous policy is retained, just reframed and reorganized. Styled entirely with `.site-*` classes added to `app/globals.css`. The "last updated" string is the `LAST_UPDATED` const in the page.
...
- `lectio session import --student ID --school ID` - Audited import of an explicitly-consented encrypted Supabase session
